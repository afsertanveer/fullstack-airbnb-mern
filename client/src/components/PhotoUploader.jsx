...
function PhotoUploader({
  ...
  const selectAsMainPhoto = (e, photo) => {
    e.preventDefault();
    setAddedPhotoes([photo, ...addedPhotoes.filter((p) => p !== photo)]);
  };
  return (
    <div>
      ...
      <div className="grid gap-2 grid-cols-3 lg:grid-cols-6 md:grid-cols-4 mt-2">
        ...
        <label className="cursor-pointer border bg-transparent rounded-xl px-8 py-8 text-4xl  flex justify-center gap-1 items-center">
          <input
            type="file"
            name="photos"
            id="photos"
